...
torch.save(model.state_dict(), '{0}/annnxlimf1.pth'.format(model_cp))
...
pred_data= list(datas[:seq])+list(model(x_train).data.reshape(-1)) + list(model(x_test).data.reshape(-1))
